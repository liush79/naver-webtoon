...
def usage():
	...
	print
	print "options:"
	print "-e <episode range>         (default: 0-9999)"
	print "-r <RSS address>"
	print "-o <output directory>      (default: ./)"
	print
	print "sample:"
	print "daumwebtoon.py -e 0-9 -r http://cartoon.media.daum.net/webtoon/rss/petermon (episode 0 ~ 9 download)"
	print
	sys.exit(2)
...
def get_imginfo(id, cookie):
	...
	dict = {}
	output_file = open('./out.output', 'r')
	dict = simplejson.loads(output_file.read(), encoding='utf8')
	output_file.close()
...
def main(argv):
	...
	for id in idlist:
		...
		if not os.path.isdir(output_dir+info['title']):
			mkdir = 'mkdir '+output_dir+info['title']
			os.system(mkdir.encode('utf-8'))
